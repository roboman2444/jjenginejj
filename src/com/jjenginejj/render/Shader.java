...
public class Shader{
	...
	private static ArrayList<String> findShaders(File dir){
		...
		for(int fragi = 0; fragi < files.length; fragi++){
			//find a frag file
			if(!files[fragi].getName().contains(".frag")){
				continue;
			}
			String fileName = files[fragi].getName().split(".frag")[0];
			//find its accompanying vert file
			for(int verti = 0; verti < files.length; verti++){
				if(files[verti].getName().equals(fileName + ".vert")) 
	...
	public static void initShader(){
		...
		for(int i=0; i< ShaderNames.size(); i++){
			...
			System.out.print("Loaded com.jjenginejj.render.Shader \"" + ShaderNames.get(i) + "\" at location " + tempShaderInt + "\n");
		}
	}
	/*
	 * With the exception of syntax, setting up com.com.jjenginejj.system.jjenginejj.system.util.vertex and fragment shaders
	 * is the same.
	 * @param the name and path to the com.com.jjenginejj.system.jjenginejj.system.util.vertex shader
	 */
